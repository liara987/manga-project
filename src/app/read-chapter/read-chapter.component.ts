...
export class ReadChapterComponent implements OnInit {
  ...
  goToDetails(): void {
    if (!this.id_manga) return;

    if (this.title && this.image) {
      this.router.navigate(['/manga', this.id_manga, this.title, this.image]);
    } else {
      this.router.navigate(['/manga', this.id_manga]);
    }
  }
  ...
  loadChapterList(): void {
    ...
    this.mangaService.getAllChapter(this.id_manga, this.lang).subscribe({
      ...
      complete: () => {
        const flat: any[] = [];
        volumeChapters.forEach((chapters) =>
          chapters.forEach((c: any) => flat.push(c)),
        );
  ...
  navigateTo(chapterId: string, others: string[], chapterNumber: number): void {
    ...
    const commands = ['/chapter', id, this.id_manga, chapterNumber, this.lang];
    const queryParams = this.title && this.image ? { title: this.title, image: this.image } : undefined;

    this.router
      .navigateByUrl(`/refreshChapter/${id}/${this.id_manga}/${chapterNumber}/${this.lang}`, {
        skipLocationChange: true,
      })
      .then(() => this.router.navigate(commands, { queryParams }));
    window.scrollTo(0, 0);
  ...
  hasPrev(): boolean {
    return this.chapterList().some(
      (c) => +c.chapter === this.chapterNumber - 1,
    );
  }
  hasNext(): boolean {
    return this.chapterList().some(
